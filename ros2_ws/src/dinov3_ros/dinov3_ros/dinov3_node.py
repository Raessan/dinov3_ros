...
class Dinov3Node(LifecycleNode):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("detection_model.model_path", '')
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.detection_model = {
            'model_path': self.get_parameter('detection_model.model_path').get_parameter_value().string_value,
        ...
        # Open models
        self.dino_backbone_loader = torch.hub.load(repo_or_dir=self.dino_model["model_path"],
                                            model=self.dino_model["model_name"],
                                            source = self.dino_model["source"])
        
        self.dino_backbone = DinoBackbone(self.dino_backbone_loader, self.dinio_model['n_layers']).to(self.device)
        ...
        if self.use_object_detection:
            ...
            self.detection_head.load_state_dict(torch.load(self.detection_model['model_path']))
